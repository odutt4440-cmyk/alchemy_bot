...
async def sudohelp(event):
    ...
    await event.reply("👑 **Sudo Control Panel**\n\n"
                      ...
                      "📣 `/gcast [text]` - Group Broadcast\n"
                      "🚫 `/ban [reason]` - Ban user\n"
                      "✅ `/unban [reason]` - Unban user\n"
                      "🛠 `/addsudo` - Manage powers\n"
                      "💎 `/addcoins [amt]` - Add coins\n"
                      "➖ `/remcoins [amt]` - Rem coins\n"
                      "📊 `/stats` - Global Stats\n"
                      "🔎 `/info [uid]` - User details")

# 2. Add/Rem Sudo Powers
@events.register(events.NewMessage(pattern=r'/addsudo'))
async def addsudo(event):
    if not await is_admin(event.sender_id): return
    target = await event.get_reply_message()
    if not target: return await event.reply("Reply to user!")
    u = await db.users.find_one({"user_id": target.sender_id})
    b = "✅" if u.get("is_banned") else "❌"
    a = "✅" if u.get("is_admin") else "❌"
    
    btns = [
        [Button.inline(f"{a} Admin", f"p_adm_{target.sender_id}"), Button.inline(f"{b} Ban", f"p_ban_{target.sender_id}")],
        [Button.inline("🚫 Clear All", f"p_clr_{target.sender_id}"), Button.inline("❌ Cancel", f"p_cancel_{target.sender_id}")]
    ]
    await event.reply(f"Managing: {target.sender_id}", buttons=btns)
...
async def power_callback(event):
    ...
    if act == "adm": await db.users.update_one({"user_id": uid}, {"$set": {"is_admin": True}})
    elif act == "ban": await db.users.update_one({"user_id": uid}, {"$set": {"is_banned": True}})
    elif act == "clr": await db.users.update_one({"user_id": uid}, {"$set": {"is_admin": False, "is_banned": False}})
...
async def coins_cmd(event):
    ...
    amt = int(event.pattern_match.group(0).split()[1])
    val = amt if "add" in event.pattern_match.group(0) else -amt
    await db.users.update_one({"user_id": reply.sender_id}, {"$inc": {"coins": val}})
    await event.reply(f"✅ Coins updated by {val}!")
