...
async def addsudo(event):
    ...
    btns = [
        ...
        [Button.inline("❌ Cancel", f"p_cancel_{target.sender_id}")]
    ]
    await event.reply(f"Managing: {target.sender_id}\n\nClick buttons to toggle (no need to close!):", buttons=btns)
...
async def coins_cmd(event):
    ...
    target_id = reply.sender_id if reply else (int(args[2]) if len(args) > 2 else None)
    
    if not amt or not target_id: return await event.reply("Usage: /addcoins [amt] (reply user) OR /addcoins [id] [amt]")
